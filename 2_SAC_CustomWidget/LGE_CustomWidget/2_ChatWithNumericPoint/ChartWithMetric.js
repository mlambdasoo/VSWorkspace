...
(function () {
  ...
  template.innerHTML = `
                <style>
                    ...
                    .metric-container {
                        position: relative;
                        width: var(--metric-container-width);
                        flex-shrink: 0;
                        text-align: center;
                    }
                    .metric-label {
  ...
  class HorizontalBarChart extends HTMLElement {
    constructor() {
      ...
      style.textContent = `
              :host {
                display: block;
                --metric-container-width: 200px;
                --metric-containers-count: ${this._metricIndices.length};
                --margin-right: 10px;
              }
            `;
    ...
    // metric indices를 설정하는 메서드
    setMetricIndices(indices) {
      this._metricIndices = indices;
      // CSS 변수 업데이트
      this.style.setProperty("--metric-containers-count", indices.length);
      // 차트 다시 렌더링
      this.render();
    }
