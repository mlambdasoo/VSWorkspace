...
const iconMap = {
  ...
  Nevada:
    "https://www.iconpacks.net/icons/4/free-school-building-icon-12440.png",
  Oregon: "https://www.iconpacks.net/icons/2/free-hotel-icon-3453.png",
  "Carbonated Drinks":
    "https://www.iconpacks.net/icons/2/free-martini-icon-1859.png",
  ...
  "Gross Margin":
    "https://www.iconpacks.net/icons/3/free-icon-gold-coins-6373.png",
  Discount:
    "https://www.iconpacks.net/icons/free-icons-6/free-red-sales-price-tag-or-discount-sticker-icon-18979.png",
  "Original Sales Price":
    "https://www.iconpacks.net/icons/2/free-icon-euro-coin-2151.png",
...
class ChartPlotareaComponent extends HTMLElement {
  ...
  render() {
    ...
    this._series.forEach((singleSeries, index) => {
      ...
      const markerSize =
        this._chartType !== "barcolumn" || !sampleDataInfo
          ? undefined
          : this._isHorizontal
          ? sampleDataInfo.height
          : sampleDataInfo.width;
  ...
  renderData(dataInfo, options) {
    ...
    if (options?.showAsTriangle) {
      ...
      width = height =
        (Math.min(originalWidth, originalHeight) / 2) * (1 + increment);
    ...
    // rounded가 true이면 roundedStyle적용하고, 아니면 backgroundStyle을 그대로 가져감.
    const barStyle = backgroundStyle;
    ...
    barColumnContainer.setAttribute(
      "style",
      `${backgroundImage} ${barStyle} position: absolute; top: ${y}px; left: ${x}px; width: ${width}px; height: ${height}px;${
  ...
  renderLabel(labelInfo, options) {
    ...
    let labelColor = this._chartType.startsWith("stacked")
      ? "#666"
      : options.color;
    ...
    // 도넛 데이터 레이블 정의
    if (
      this._donutDataLabel &&
      this._chartType === "barcolumn" &&
      options.markerSize > 0 &&
      !isNaN(options.totalSeriesValue)
    ) {
      ...
      const top = this._isHorizontal
        ? y + height / 2 - markerSize / 2
        : y + height - markerSize;
      ...
      const strokeWidth = markerSize * 0.12 * donutLabelSizeRadio;
      const circleRadius =
        ((markerSize - strokeWidth) / 2) * donutLabelSizeRadio;
      const circleAttr = {
        cx: labelSvgSize / 2,
        cy: labelSvgSize / 2,
        r: circleRadius,
      ...
      dCommands.push(
        `a ${outterR} ${outterR} 0 ${largeArc} 1 ${outterR * Math.sin(theta)} ${
          outterR * (1 - Math.cos(theta))
        }`
      );
      dCommands.push(
        `l ${-strokeWidth * Math.sin(theta)} ${strokeWidth * Math.cos(theta)}`
      );
      dCommands.push(
        `A ${innerR} ${innerR} 0 ${largeArc} 0 ${maxR} ${maxR - innerR}`
      );
      dCommands.push(`Z`);
      const pathAttr = {
        fill: labelColor,
      ...
      // 도넛안에 들어가는 값으로, custom unit이 있다면 적용하고, 아니라면 백만 단위로 적용
      var textInMillian;
      if (false) {
        //
        let resultValue = (labelInfo.pointValue / this._customUnitNum).toFixed(
          0
        );
        textInMillian = `${resultValue} ${this._customUnitName}`;
      } else textInMillian = getTextInMillian(pointValue);
      ...
      // 일반적인 케이스 custom unit이 켜져 있으면 숫자 유형 적용
      labelElement = document.createElement("span");
      labelElement.classList.add("common-label");
      labelElement.setAttribute(
        "style",
        `background-color: ${bgColor}; position: absolute; top: ${y}px; left: ${x}px; width: ${
          this._size.width - x
        }px; height: ${height}px; color: ${color}; font-size: ${fontSize}; text-align: center`
      );
      ...
      if (
        !(this._inclined && this._chartType === "line") &&
        (hidden || outOfViewport)
      ) {
        return;
      }
    ...
    if (this._inclined && this._chartType === "line") {
      ...
      if (this.flag) labelElement.style.top = `${y + 20}px`;
      this.flag = !this.flag;
    }
  ...
  renderAxisLabel(label) {
    ...
    labelContainer.setAttribute(
      "style",
      `background-color: ${bgColor}; width: ${width + 36}px; left: ${
        x - 30
      }px; top: ${y - 2}px; font-size: ${fontSize};`
    );
    ...
    iconImg.setAttribute(
      "src",
      iconMap[pointValue] || iconMap.City || iconMap.Info
    );

    if (!this._axisLabelIcon) iconImg.style.display = "none";
  }
  ...
  renderAxisStackLabel(stackLabelInfo) {
    ...
    const bgColor = "transparent";
    const { x, y, width, height, formattedValue, fontSize } = stackLabelInfo;
